...
public void Event_SendToLocation(Event event, const char[] name, bool dontBroadcast)
{
    if(GetConVarBool(g_hSaveLocCvar))
    {
        ...
        CreateTimer(0.5, Timer_SendToLocation, userid);
    }
}
...
{
    if(GetEngineTime() - g_fLastSaveLocTime[client] < 1.0)
    {
        return false;
    }
...
void SaveLocation(int client, const char[] event)
{
    ...
    {
        CPrintToChat(client, "%t", "Saved location", g_msg_start, g_msg_textcol);
    }
...
LoadSaveLocation(client, index)
{
    ...
    CPrintToChat(client, "%t", "Loaded saved location", g_msg_start, g_msg_textcol);
}
