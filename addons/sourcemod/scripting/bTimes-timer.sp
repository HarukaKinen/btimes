...
Handle g_fwdOnPrintFinishMessage;
...
public void OnPluginStart()
{
    ...
    {
        Handle hGameData = LoadGameConfigFile("btimer.games");
        ...
        if(hGameData != null)
        {
            ...
            {
                SetFailState("Couldn't get the offset for \"GetPlayerMaxSpeed\"");
            }
        }
        delete hGameData;
    }
...
public void OnAllPluginsLoaded()
{
    ...
    g_bZonePluginLoaded = LibraryExists("timer-zones");
...
public void OnLibraryRemoved(const char[] library)
{
    ...
    else if(StrEqual(library, "timer-zones"))
    {
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
    ...
    CreateNative("Timer_SetClientTimerInfo", Native_SetClientTimerInfo);
    ...
    CreateNative("Style.GetTranslation",          Native_StyleGetTranslation);
    CreateNative("Style.HasTranslation",          Native_StyleHasTranslation);
    ...
    g_fwdOnPrintFinishMessage = CreateGlobalForward("OnPrintFinishMessage", ET_Event, Param_Cell, Param_Float, Param_Cell, Param_Cell, Param_Cell, Param_Cell, Param_Float, Param_Cell, Param_Cell, Param_Cell, Param_Cell, Param_Float, Param_Float, Param_String, Param_Cell);
...
{
    if(!pThis || !IsPlayerAlive(pThis) || !IsClientInGame(pThis) || IsFakeClient(pThis) || IsClientSourceTV(pThis))
    {
        return MRES_Ignored;
    }

    DHookSetReturn(hReturn, view_as<float>(g_StyleConfig[g_TimerInfo[pThis].GetStyle(g_TimerInfo[pThis].Type)].Auto ? 260.0 : 250.0));
...
{
    if(Style(oldStyle).AirAcceleration != Style(style).AirAcceleration && IsClientInGame(client))
    {
        PrintColorText(client, "%t", "AA Change", 
            g_msg_start,
            g_msg_textcol,
            g_msg_varcol,
...
{
    if (StrEqual(cvarName, "fps_max"))
    {
        float fps = StringToFloat(cvarValue);
        ...
        if(sArgs[0] == '!' || sArgs[0] == '/')
        {
            ...
            for(int style; style < g_TotalStyles; style++)
            {
                ...
                if(Style(style).Enabled && Style(style).GetAllowType(g_TimerType[client]) && g_StyleConfig[style].Selectable)
                {
                    ...
                    Style(style).GetCommands(sStyle, sizeof(sStyle));
                    char sStyleCommands[32][128];
                    int iCommands = ExplodeString(sStyle, ";", sStyleCommands, 32, 128, false);

                    for(int i = 0; i < iCommands; i++)
                    {
                        TrimString(sStyleCommands[i]);
                        StripQuotes(sStyleCommands[i]);

                        if(StrEqual(sArgs2, sStyleCommands[i]))
                        {
...
public int Native_LoadTimes(Handle plugin, int numParams)
{
    DB_LoadTimes();
}
...
public void ShowTimeFromMapName_Callback(Handle owner, Handle hndl, const char[] error, DataPack pack)
{
    if(hndl != INVALID_HANDLE)
    {
        ...
        if(client != 0)
        {
            ...
            {
                PrintColorText(client, "%t", "ShowTimeFromMapName_Failed",
                    g_msg_start,
                    g_msg_textcol);
            }
        }
...
public int Native_StyleGetCommands(Handle plugin, int numParams)
{
    SetNativeString(2, g_StyleCommands[GetNativeCell(1)], GetNativeCell(3));
}

public int Native_StyleGetTranslation(Handle plugin, int numParams)
{
    SetNativeString(2, g_StyleTranslation[GetNativeCell(1)], GetNativeCell(3));
}
...
public int Native_StyleHasTranslation(Handle plugin, int numParams)
{
    int len;
    GetNativeStringLength(2, len);
    len++;
    ...
    GetNativeString(2, buffer, len);
    int delimiterCount, style = GetNativeCell(1);
    int specialKeyLen = strlen(g_StyleTranslation[style]) + 1;
    for(int idx; idx < specialKeyLen; idx++)
    {
        if(g_StyleTranslation[style][idx] == ';')
            delimiterCount++;
    }
    
    char[][] specialKeyList = new char[delimiterCount + 1][32];
    ExplodeString(g_StyleTranslation[style], ";", specialKeyList, delimiterCount + 1, 32, false);
...
public void OnAutoStopsTimerChanged(ConVar convar, const char[] oldValue, const char[] newValue)
{
    if(StringToInt(newValue) == 1)
    {
        for(int client = 1; client <= MaxClients; client++)
        {
            if(IsClientInGame(client) && IsBeingTimed(client, TIMER_ANY) && !IsFakeClient(client))
            {
                StopTimer(client);
            }
        }
...
public void OnOvertakeSuccess(Database db, any data, int numQueries, Handle[] results, bool[] isOvertake)
{
    ...
    if(client != 0)
    {
        if(isOvertake[0] == true)
        {
            ...
            PrintColorText(client, "%t", "OverTake 1",
                g_msg_start,
                g_msg_textcol,
                g_msg_varcol,
                recordsStolen,
            ...
            while(SQL_FetchRow(results[0]))
            {
                ...
                FormatPlayerTime(fTheirTime, sTheirTime, sizeof(sTheirTime), 2);
                FormatPlayerTime(fYourTime, sYourTime, sizeof(sYourTime), 2);
                GetNameFromPlayerID(overtaker, sOvertaker, sizeof(sOvertaker));
                
                FormatEx(sDisplay, sizeof(sDisplay), "%t", "OverTake 3",
...
public Action SM_WR(int client, int args)
{
    ...
    else
    {
        ...
        {
            PrintColorText(client, "%t", "SearchWr_Failed",
                g_msg_start,
                g_msg_varcol,
                sArg,
                g_msg_textcol);
...
void ShowWorldRecordMenu(int client, int type, int tas, const char[] sMap)
{
    ...
    char sType[32], sStyle[32], sInfo[128], sTrans[64];
    
    GetTypeName(type, sType, sizeof(sType));
    Menu menu = new Menu(Menu_WRStyleSelection);
    menu.SetTitle("%t", "ShowWorldRecordMenu", sMap, sType, tas?"(TAS)":"");
    ...
    if(AreClientCookiesCached(client) && GetCookieBool(client, g_hPersonalizedStyleList) && g_bFavoriteStylesLoaded[client] == true && g_hFavoriteStyles[client][type][tas].Length > 0)
    {
        ...
        for(int idx; idx < g_hFavoriteStyles[client][type][tas].Length; idx++)
        {
            ...
            if(g_StyleConfig[style].Enabled && g_StyleConfig[style].GetAllowType(type) && g_StyleConfig[style].AllowTAS)
            {
                g_StyleConfig[style].GetName(sStyle, sizeof(sStyle));
                g_StyleConfig[style].GetTranslation(sTrans, sizeof(sTrans));
                if(strlen(sTrans) > 0)
                    FormatEx(sStyle, sizeof(sStyle), "%s(%s)", sStyle, sTrans);
                else
        ...
        for(int style; style < g_TotalStyles; style++)
        {
            if(g_StyleConfig[style].Enabled && g_StyleConfig[style].GetAllowType(type) && (!tas || g_StyleConfig[style].AllowTAS) && styleUsed[style] == false)
            {
                g_StyleConfig[style].GetName(sStyle, sizeof(sStyle));
                g_StyleConfig[style].GetTranslation(sTrans, sizeof(sTrans));
                if(strlen(sTrans) > 0)
...
public void ShowRecordList_Success(Database db, DataPack pack, int numQueries, Handle[] hndl, any[] queryData)
{
    ...
    if(SQL_MoreRows(hndl[1]))
    {
        ...
        if(tas == 0)
        {
            FormatEx(sMode, sizeof(sMode), "%t","Default");
        }
        else
        {
        ...
        menu.SetTitle("%t", "ShowRecordList",sMap, sType, sStyle, sMode);
        ...
        while(SQL_FetchRow(hndl[1]))
        {
            ...
            FormatPlayerTime(fTime, sTime, sizeof(sTime), 2);
    ...
    {
        PrintColorText(client, "%t", "ShowRecordList_NoRecord",
            g_msg_start,
            g_msg_textcol);
...
public Action SM_Time(int client, int args)
{
    if(args == 0)
    {
        ...
        {
            PrintColorText(client, "%t", "Time_NotLoaded",
                g_msg_start,
                g_msg_textcol);
        }
    }
    else if(args == 1)
    {
        ...
        {
            PrintColorText(client, "%t", "Time_NoMapOrPlayer",
                g_msg_start,
                g_msg_textcol,
                g_msg_varcol,
                sArg);
...
void OpenShowTimeMenu(int client, int playerId, const char[] sMap)
{
    ...
    menu.SetTitle("%t", "OpenShowTimeMenu");
...
public void ShowTime_Success(Database db, DataPack data, int numQueries, Handle[] hndl, any[] queryData)
{
    ...
    if(client != 0)
    {
        ...
        {
            FormatPlayerTime(SQL_FetchFloat(hndl[0], 0), sTime, sizeof(sTime), 2);
...
public int Panel_TimeInfo(Menu menu, MenuAction action, int param1, int param2)
{
    if(action == MenuAction_Select)
    {
        if(param2 == 1)
        {
            Ranks_OpenStatsMenu(param1, g_ShowTime_PlayerId[param1], g_ShowTime_Type[param1], g_ShowTime_Style[param1]);
        }
        else if(param2 == 2)
        {
...
void OpenStyleMenu(int client, int type, const char[] title)
{
    ...
    char sStyle[32], sInfo[32], sStyleTrans[64];
    if(AreClientCookiesCached(client) && GetCookieBool(client, g_hPersonalizedStyleList) && g_bFavoriteStylesLoaded[client] == true && g_hFavoriteStyles[client][type][tas].Length > 0)
    {
        char sDisplay[64];
        menu.SetTitle("%t", "OpenStyleMenu", title);
        ...
        for(int idx; idx < g_hFavoriteStyles[client][type][tas].Length; idx++)
        {
            ...
            {
                g_StyleConfig[style].GetName(sStyle, sizeof(sStyle));
                g_StyleConfig[style].GetTranslation(sStyleTrans, sizeof(sStyleTrans));
                if(strlen(sStyleTrans) > 0)
                {
                    FormatEx(sStyle, sizeof(sStyle), "%s(%s)", sStyle, sStyleTrans);
                }
                FormatEx(sDisplay, sizeof(sDisplay), "%s", sStyle, sStyleTrans);
        ...
        for(int style; style < g_TotalStyles; style++)
        {
            if(g_StyleConfig[style].Enabled && g_StyleConfig[style].GetAllowType(type) && g_StyleConfig[style].Selectable && styleUsed[style] == false)
            {
                ...
                FormatEx(sDisplay, sizeof(sDisplay), "%s", sStyle);
                FormatEx(sInfo, sizeof(sInfo), "%d;%d", type, style);
                menu.AddItem(sInfo, sStyle);
            }
        }
...
{
    char sTitle[32];
    FormatEx(sTitle, sizeof(sTitle), "%t", "ChangeStyle");
    OpenStyleMenu(client, TIMER_MAIN, sTitle);
...
{
    char sTitle[32];
    FormatEx(sTitle, sizeof(sTitle), "%t", "ChangeBonusStyle");
    OpenStyleMenu(client, TIMER_BONUS, sTitle);
...
public Action SM_Practice(int client, int args)
{
    if(GetConVarBool(g_hAllowNoClip))
    {
        ...
        else
        {
            ...
            if(!(0 <= fSpeed <= 10))
            {
                PrintColorText(client, "%t", "NoclipSpeed_Error",
                    g_msg_start,
                    g_msg_textcol);
            ...
            PrintColorText(client, "%t", "NoclipSpeed",
                g_msg_start,
                g_msg_textcol,
                g_msg_varcol,
                fSpeed,
...
void PauseTimer(int client)
{
    if(GetConVarBool(g_hAllowPause))
    {
        if(!g_bZonePluginLoaded || (Timer_InsideZone(client, MAIN_START) == -1 && Timer_InsideZone(client, BONUS_START) == -1))
        {
            if(g_TimerInfo[client].IsTiming == true)
            {
                if(g_TimerInfo[client].Paused == false)
                {
                    ...
                    {
                        PrintColorText(client, "%t", "Pause_Air",
                            g_msg_start,
                            g_msg_textcol);
                    }
                }
                ...
                {
                    PrintColorText(client, "%t", "Pause_Paused",
                        g_msg_start,
                        g_msg_textcol);
                }
            }
            ...
            {
                PrintColorText(client, "%t", "Pause_NoTimer",
                    g_msg_start,
                    g_msg_textcol);
            }
        }
        ...
        {
            PrintColorText(client, "%t", "Pause_StartZone",
                g_msg_start,
                g_msg_textcol);
        }
    }
...
void UnpauseTimer(int client)
{
    if(g_TimerInfo[client].IsTiming == true)
    {
        ...
        {
            PrintColorText(client, "%t", "UnPause_NotPaused",
                g_msg_start,
                g_msg_textcol);
        }
    }
...
void RR_StartMapSelection(int client)
{
    ...
    menu.SetTitle("%t", "RR_SelectMap");
    menu.AddItem("all;", "All Maps");
    
    char sCurrent[PLATFORM_MAX_PATH];
    FormatEx(sCurrent, sizeof(sCurrent), "%t", "RR_MapName",g_sMapName);
...
void RR_StartActiveRecordSelection(int client)
{
    ...
    menu.SetTitle("%t", "RR_ActiveRecord");
    menu.AddItem("yes", "Yes");
    menu.AddItem("no", "No");
    menu.Display(client, MENU_TIME_FOREVER);
}
...
public Action OnTimerStart_Pre(int client, int Type, int style, int Method)
{
    ...
    char sWarningMsg[128], sWarningMsg_Fps[128];
    ...
    if(GetEntPropFloat(client, Prop_Data, "m_flLaggedMovementValue") != 1.0)
    {
        if(g_bTasPluginLoaded)
        {
            ...
            {
                FormatEx(sWarningMsg, sizeof(sWarningMsg), "%t", "Warning_MovementSpeed", 
                    g_msg_start,
                    g_msg_textcol,
                    g_msg_varcol,
                    g_msg_textcol);
                WarnClient(client, sWarningMsg, 30.0);
                return Plugin_Handled;
            }
        }
        else
    ...
    {
        FormatEx(sWarningMsg_Fps, sizeof(sWarningMsg_Fps), "%t", "Warning_Fpsmax", 
            g_msg_start,
            g_msg_textcol,
            g_msg_varcol,
            g_StyleConfig[style].MinimumFPS,
            g_msg_textcol);
        WarnClient(client, sWarningMsg_Fps, 30.0);
...
public Action OnTimerFinished_Pre(int client, int Type, int style)
{
    ...
    {
        PrintColorText(client, "%s%sYour time did not save as you are %stimerbanned%s.", g_msg_start, g_msg_textcol, g_msg_varcol, g_msg_textcol);
    }
    ...
    if(g_StyleConfig[style].IsSpecial == true)
    {
        if(g_StyleConfig[style].HasSpecialKey("sw"))
        {
            ...
            {
                PrintColorText(client, "%t", "SideWayAc",
                    g_msg_start,
                    g_msg_textcol);
                StopTimer(client);
                return Plugin_Handled;
...
public int Native_FinishTimer(Handle plugin, int numParams)
{
    ...
    int style  = g_TimerInfo[client].GetStyle(g_TimerInfo[client].Type);
    int tas    = g_bTasPluginLoaded?view_as<int>(TAS_InEditMode(client)):0;
    float fOldTime = g_fTime[client][Type][style][tas];
    float fOldWRTime = Timer_GetTimeAtPosition(Type, style, tas, 0);
    float fTime = g_TimerInfo[client].CurrentTime;
    int jumps = g_TimerInfo[client].Jumps;
    int strafes = g_TimerInfo[client].Strafes;
    float sync = g_TimerInfo[client].Sync;
    ...
    if(fResult != Plugin_Handled)
    {
        ...
        Call_StartForward(g_fwdOnPrintFinishMessage);
        Call_PushCell(client);
        Call_PushFloat(fTime);
        Call_PushCell(Type);
        Call_PushCell(style);
    ...
    if(kv != INVALID_HANDLE)
    {
        ...
        do
        {
            ...
            if(keyExists == true)
            {
                ...
                kv.GetString("commands", sBuffer, sizeof(sBuffer));
                g_StyleConfig[key].SetCommands(sBuffer);
                ...
                kv.GetString("translation", sBuffer, sizeof(sBuffer));
                g_StyleConfig[key].SetTranslation(sBuffer, sizeof(sBuffer));
...
public int Native_SetClientTimerInfo(Handle plugin, int numParams)
{
    int client = GetNativeCell(1);
    TimerInfo t = GetNativeCell(2);

    g_TimerInfo[client].IsTiming = t.IsTiming;
    g_TimerInfo[client].Paused = t.Paused;
    g_TimerInfo[client].CurrentTime = t.CurrentTime;
    g_TimerInfo[client].TotalSync = t.TotalSync;
    g_TimerInfo[client].GoodSync = t.GoodSync;
    g_TimerInfo[client].CheckpointsUsed = t.CheckpointsUsed;
    g_TimerInfo[client].Jumps = t.Jumps;
    g_TimerInfo[client].Strafes = t.Strafes;
    g_TimerInfo[client].SetStyle(t.Type, t.ActiveStyle);
}
...
void OpenDeleteRecordsTimeListSelectMax(int client)
{
    ...
    FormatPlayerTime(fTime_Min, sTime_Min, sizeof(sTime_Min), 2);
    ...
    FormatPlayerTime(fTime_Max, sTime_Max, sizeof(sTime_Max), 2);
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2])
{
    ...
    if(IsPlayerAlive(client) && !IsFakeClient(client))
    {
        ...
        // Unrestrict movement in noclip / on ground / on ladder
        // TODO: recode me later
        if(GetEntityMoveType(client) == MOVETYPE_NOCLIP || GetEntityMoveType(client) == MOVETYPE_LADDER || (GetEntityFlags(client) & FL_ONGROUND))
            bRestrict = false;
        ...
        {        
            ...
            if(bStop == true)
            {
                ...
                if(!g_bZonePluginLoaded || (Timer_InsideZone(client, MAIN_START) == -1 && Timer_InsideZone(client, BONUS_START) == -1))
                {
                    PrintColorText(client, "%t", "Warning_leftright",
                        g_msg_start,
                        g_msg_textcol);
                }
            }
        ...
        if(g_TimerInfo[client].IsTiming == true)
        {
            ...
            {
                StopTimer(client);
                PrintColorText(client, "%t", "Warning_noclip",
                    g_msg_start,
                    g_msg_textcol);
            }
        }
        ...
        {
            if(g_StyleConfig[style].Auto || (g_bZonePluginLoaded && g_StyleConfig[style].Freestyle && Timer_InsideZone(client, FREESTYLE, 1 << style, view_as<int>(zFs_Auto)) != -1))
            {
                if(IsPlayerAlive(client))
                {
                    if(buttons & IN_JUMP)
