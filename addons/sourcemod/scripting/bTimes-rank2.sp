...
//ArrayList g_hStyleWRRank[MAX_TYPES][MAX_STYLES];
ArrayList g_hMaps;
ArrayList g_hTiers;
...
int g_iMVPs_offset;
int g_iLastUsedPlayerID[MAXPLAYERS + 1];
int g_iSearchFailedTimes[MAXPLAYERS + 1];
...
public void OnPluginStart()
{
    ...
    for(int Type; Type < MAX_TYPES; Type++)
    {
        for(int style; style < MAX_STYLES; style++)
        {
            ...
            //g_hStyleWRRank[Type][style] = new ArrayList(2);
        }
    }
    ...
    LoadTranslations("btimes-rank.phrases");
}
...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
    ...
    CreateNative("Ranks_OpenStatsMenu",              Native_OpenStatsMenu);
...
public void OnClientDisconnect(int client)
{
    ...
    g_WRCount[client]                = 0;
    g_WRRank[client]                 = 0;
    g_iLastUsedPlayerID[client]      = 0;
    g_iSearchFailedTimes[client]     = 0;
...
public bool SmartMessage_OverallRank(int client)
{
    if(g_bClientRanksAreLoaded[client])
    {
        ...
        {
            PrintColorText(client, "%t", "RankOverall",
                g_msg_start,
                g_msg_textcol,
                g_msg_varcol,
        ...
        {
            PrintColorText(client, "%t", "RankOverall_NotRanked",
                g_msg_start,
                g_msg_textcol);
        }
        return true;
...
public bool SmartMessage_StyleRank(int client)
{
    ...
    if(IsPlayerAlive(client) && IsBeingTimed(client, TIMER_ANY))
    {
        ...
        if(!tas)
        {
            ...
            {
                PrintColorText(client, "%t", "RankStyle",
                    g_msg_start,
                    g_msg_textcol,
                    g_msg_varcol,
            ...
            {
                PrintColorText(client, "%t", "RankStyle_NotRanked",
                    g_msg_start,
                    g_msg_textcol);
            }
...
public Action SM_Rank(int client, int args)
{
    if(args == 0)
    {
        ...
        {
            PrintColorText(client, "%t", "Rank_IDHasntLoaded",
                g_msg_start,
                g_msg_textcol);
        }
    }
    else
    {
        ...
        if(target != -1)
        {
            ...
            {
                PrintColorText(client, "%t", "Rank_TargetsIDHasntLoaded",
                    g_msg_start,
                    g_msg_textcol,
                    g_msg_varcol,
                    target,
        ...
        {
            PrintColorText(client, "%t", "Rank_CantFindPlayer",
                g_msg_start,
                g_msg_textcol,
                g_msg_varcol,
                sName,
...
void OpenRankMenu(int client, int playerId)
{
    ...
    menu.SetTitle("%t", "Choose Rank Type");
    
    char sInfo[32], sDisplay[64], sIdk[64], sTrans[64];
    
    FormatEx(sInfo, sizeof(sInfo), "overall;%d", playerId);
    FormatEx(sIdk, sizeof(sIdk), "%t", "Overall");
    menu.AddItem(sInfo, sIdk);
            ...
            {    
                ...
                switch(Type)
                {
                    case TIMER_MAIN:
                    {
                        ...
                        Style(style).GetTranslation(sTrans, sizeof(sTrans));
                        if(strlen(sTrans) > 0)
                            FormatEx(sDisplay, sizeof(sDisplay), "%s(%s)", sDisplay, sTrans);
                        else
                            FormatEx(sDisplay, sizeof(sDisplay), "%s", sDisplay);
    ...
    {    
        ...
        if(client != 0)
        {
            ...
            if(SQL_GetRowCount(hndl))
            {
                ...
                PrintColorText(client, "%t", "ShowRank_Succeed",
                    g_msg_start,
                    g_msg_varcol,
                    sName,
                    g_msg_textcol,
            ...
            {
                PrintColorText(client, "%t", "ShowRank_Failed",
                    g_msg_start,
                    g_msg_textcol);
            }
...
void OpenTopMenu(int client)
{
    ...
    menu.SetTitle("%t", "Choose Rank List Type");
        ...
        if(client != 0)
        {
            if(SQL_GetRowCount(hndl))
            {
                ...
                while(SQL_FetchRow(hndl) && rowsAdded < 100)
                {
                    ...
                    // Add data to menu
                    FormatEx(sInfo, sizeof(sInfo), "%d", playerId);
            ...
            {
                PrintColorText(client, "%t", "ShowTop_NoBodyRank",
                    g_msg_start,
                    g_msg_textcol);
            }
        }
...
public int Menu_TopList(Menu menu, MenuAction action, int client, int param2)
{
    if(action == MenuAction_Select)
    {
        ...
        int playerId = StringToInt(sInfo);
        g_iLastUsedPlayerID[client] = playerId;
        ShowPlayerStats(client, playerId);
    }
    else if(action == MenuAction_End)
    {
        delete menu;
...
public Action SM_Tier(int client, int args)
{
    ...
    else
    {
        ...
        {
            PrintColorText(client, "%t", "Tier_Error",
                g_msg_start,
                g_msg_varcol,
                sArg,
                g_msg_textcol);
    ...
    {    
        ...
        if(client != 0)
        {
            ...
            if(SQL_FetchRow(hndl))
            {
                ...
                PrintColorText(client, "%t", "Tier_Set",
                    g_msg_start,
                    g_msg_varcol,
                    sMap,
                    g_msg_textcol,
            ...
            {
                PrintColorText(client, "%t", "Tier_NotSet",
                    g_msg_start,
                    g_msg_varcol,
                    sMap,
                    g_msg_textcol);
...
public Action SM_SetTier(int client, int args)
{
    ...
    else if(args == 1)
    {
        ...
        else if(1 <= StringToInt(sArg) <= 12)
        {
            SetMapTier(client, g_sMapName, StringToInt(sArg));
        }
        else
        {
            PrintColorText(client, "%t", "Tier_Setup",
                g_msg_start,
                g_msg_varcol,
                sArg,
                g_msg_textcol);
    ...
    else if(args == 2)
    {
        ...
        {
            PrintColorText(client, "%t", "Tier_Error_Map",
                g_msg_start,
                g_msg_varcol,
                sMap,
                g_msg_textcol);
        ...
        if(tier < 1 || tier > 12)
        {
            PrintColorText(client, "%t", "Tier_Error_Tier",
                g_msg_start,
                g_msg_varcol,
                sTier,
                g_msg_textcol);
...
public void SetMapTier_Callback(Handle owner, Handle hndl, const char[] error, DataPack pack)
{
    if(hndl != INVALID_HANDLE)
    {
        ...
        if(SQL_GetAffectedRows(hndl) == 0)
        {
            if(client != 0)
            {
                PrintColorText(client, "%t", "Tier_Callback_Error",
                    g_msg_start,
                    g_msg_varcol,
                    sMap,
                    g_msg_textcol,
        ...
        else
        {
            if(client != 0)
            {
                PrintColorText(client, "%t", "Tier_Callback_Change",
                    g_msg_start,
                    g_msg_textcol,
                    g_msg_varcol,
                    sMap,
                ...
                PrintColorText(client, "%t", "Tier_Callback_Change_Point",
                    g_msg_start,
                    g_msg_textcol);
            }
...
{
    char sArg[MAX_NAME_LENGTH], sIdk[64];
    FormatEx(sIdk, sizeof(sIdk), "%t", "MapsDone_CreatePlayerListFromName");
    GetCmdArgString(sArg, sizeof(sArg));
    
    DB_CreatePlayerListFromName(client, sIdk, g_DB, false, sArg, MapsDone_PlayerSelectedCallback);
...
{
    char sIdk[64];
    FormatEx(sIdk, sizeof(sIdk), "%t", "MapsDone_OpenMapsDoneMainMenu");
    CreateTimerSelection(client, sIdk, 0, 0, 0, false, MapsDone_TimerSelectCallback, playerId);
}
...
public void MapsDone_Success(Database db, DataPack pack, int numQueries, Handle[] results, any[] queryData)
{
    ...
    if(SQL_GetRowCount(results[1]) > 0)
    {
        ...
        {
            menu.SetTitle("%t", "MapsDone_Success_Main", sName, SQL_GetRowCount(results[1]), Timer_GetMapCycleSize());
        }
        else if(queryData[1] == TIMER_BONUS)
        {
            menu.SetTitle("%t", "MapsDone_Success_Bonus", sName, SQL_GetRowCount(results[1]));
        }
    ...
    {
        PrintColorText(client, "%t", "MapsDone_Success_NotCompleted",
            g_msg_start,
            g_msg_varcol,
            sName,
            g_msg_textcol);
    ...
    if(hndl != INVALID_HANDLE)
    {
        ...
        if(client != 0)
        {
            if(SQL_GetRowCount(hndl))
            {
                ...
                Menu menu = new Menu(Menu_TopList);
                menu.SetTitle("%t", "Top Players by Record Count");
                while(SQL_FetchRow(hndl))
                {
                    rowsAdded++;
                    ...
                    FormatEx(sDisplay, sizeof(sDisplay), "#%d: %s (%d %t)", 
                        rowsAdded,
                        sName,
                        recordCount,
                        "records");
...
public void OnClientOverallRankChanged(int client, int oldRank, int newRank)
{
    if(newRank < oldRank)
    {
        PrintColorText(client, "%t", "RankOverall_Rankup",
            g_msg_start,
            g_msg_textcol,
            g_msg_varcol,
            newRank,
    ...
    {
        PrintColorText(client, "%t", "RankOverall_Derank",
            g_msg_start,
            g_msg_textcol,
            g_msg_varcol,
            oldRank,
...
void ShowPlayerStats(int client, int PlayerID, int type = 0, int style = 0)
{
    // What we need:
    // For overall stats 
    // PlayerName  SteamID  LastConnection  PlayTime(will add this back soon)
    // MapCompletion( maps player has finished / total maps )  WRCount  Rank

    // For style stats
    // MapCompletion  CurrentMapTime  StyleRank?

    // To be implemented...
    g_iLastUsedPlayerID[client] = PlayerID;
    char sQuery[512];
    FormatEx(sQuery, sizeof(sQuery), "SELECT p.SteamID, p.User, p.LastConnection, ro.Rank, ro.Points, rs.Type, rs.Style, rs.Rank, rs.Points FROM players p \
                                      INNER JOIN ranks_overall ro ON p.PlayerID = ro.PlayerID \
                                      INNER JOIN (SELECT * FROM ranks_styles WHERE PlayerID = %d AND Type = %d AND Style = %d) as rs ON p.PlayerID = rs.PlayerID \
                                      WHERE p.PlayerID = %d",
                                      PlayerID, type, style, PlayerID);
    g_DB.Query(SQL_ShowPlayerStats_Callback, sQuery, GetClientSerial(client), DBPrio_Low);
}

public void SQL_ShowPlayerStats_Callback(Database db, DBResultSet results, const char[] error, any data)
{
    if(results == null)
    {
        Timer_Log(true, "[SQL_ShowPlayerStats_Callback] SQL query error. Reason: %s.", error);
        return;
    }
    ...
    if(!client)
    {
        Timer_Log(true, "[SQL_ShowPlayerStats_Callback] Error, client serial is null");
        return;
    }
    ...
    if(results.FetchRow())
    {
        ...
        int iLastConnection = results.FetchInt(2);
        char sLastConnection[64];
        FormatTime(sLastConnection, sizeof(sLastConnection), "%Y-%m-%d %H:%M:%S", iLastConnection);
        Format(sLastConnection, sizeof(sLastConnection), "Last connection: %s", (iLastConnection != -1) ? sLastConnection : "None");

        int iRank = results.FetchInt(3);
        float flPoints = results.FetchFloat(4);

        int iType = results.FetchInt(5);
        int iStyle = results.FetchInt(6);
        int iStyleRank = results.FetchInt(7);
        float flStylePoint = results.FetchFloat(8);

        char sTypeName[8], sStyleName[64];
        GetTypeName(iType, sTypeName, sizeof(sTypeName));
        Style(iStyle).GetName(sStyleName, sizeof(sStyleName));

        Menu menu = new Menu(StatsMenu_Handle);
        menu.SetTitle("%s's stats (%s)\n \n%s\n \n"...
                      "Overall Stats:\n    WR Count: %d\n    Rank: %d / %d (%.f pts)\n \n"...
                      "Stats on %s - %s:\n    Rank: %d / %d (%.f pts)\n \n",
                      sName, sSteamID,
                      sLastConnection,
                      Ranks_GetClientOverallRecordRank(client),
                      iRank, Ranks_GetTotalOverallRanks(), flPoints,
                      sTypeName, sStyleName,
                      iStyleRank, Ranks_GetTotalStyleRanks(iType, iStyle), flStylePoint
                      );
        
        char sInfo[32], sDisplay[64], sType[8], sStyle[64];
        for(int type = 0; type < MAX_TYPES; type++)
        {
            for(int style = 0; style < GetTotalStyles(); style++)
            {
                Format(sInfo, sizeof(sInfo), "%d|%d", type, style);

                GetTypeName(type, sType, sizeof(sType));
                Style(style).GetName(sStyle, sizeof(sStyle));
                Format(sDisplay, sizeof(sDisplay), "%s | %s", sType, sStyle);
                menu.AddItem(sInfo, sDisplay);
            }
        }
    ...
    {
        g_iSearchFailedTimes[client]++;
        if(g_iSearchFailedTimes[client] < 2)
        {
            PrintColorText(client, "%s%sFailed to search player stats, now showing default stats.", g_msg_start, g_msg_textcol);
            ShowPlayerStats(client, g_iLastUsedPlayerID[client]);
        }
        else 
        {
            PrintColorText(client, "%s%sDefault stats is empty, now stop showing stats.", g_msg_start, g_msg_textcol);
        }
    }
}

public int StatsMenu_Handle(Menu menu, MenuAction action, int client, int param2)
{
    if(action == MenuAction_Select)
    {
        char sInfo[64];
        ...
        int type  = StringToInt(sInfoExploded[0]);
        int style = StringToInt(sInfoExploded[1]);
        ShowPlayerStats(client, g_iLastUsedPlayerID[client], type, style);
    }
    if(action & MenuAction_End)
    {
        delete menu;
...
{
    ShowPlayerStats(GetNativeCell(1), GetNativeCell(2), GetNativeCell(3), GetNativeCell(4));
}